...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.ethstats == "" {
		log.Error("No kdstats server configured")
		return
	}
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{port: 31409, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{port: 31409, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	existed := err == nil
