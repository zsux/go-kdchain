...
ADD kdstats.json /kdstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../kd-net-intelligence-api && pm2 start /kdstats.json)' >  explorer.sh && \
...
// explorerEthstats is the configuration file for the kdstats javascript client.
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.kdchain
    environment:
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	files[filepath.Join(workdir, "kdstats.json")] = ethstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.kdchain"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
