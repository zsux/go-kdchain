...
import (
	...
	"github.com/kdchain/go-kdchain/les/flowcontrol"
...
func NewLesServer(kd *kd.KDchain, config *kd.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(kd.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, kd.EventMux(), kd.Engine(), newPeerSet(), kd.BlockChain(), kd.TxPool(), kd.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(kd.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(kd.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(kd.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(kd.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(kd.ChainDb())
	return srv, nil
}
...
func newCostStats(db kddb.Database) *requestCostStats {
