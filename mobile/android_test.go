...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.kdchain", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-kdchain issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogkdchainer
	...
	workspace, err := ioutil.TempDir("", "gkdchain-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/kdchain/gkdchaintest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.kdchain", "github.com/kdchain/go-kdchain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gkdchain.aar"), "gkdchain.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "kdchain", "gkdchaintest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.kdchain.gkdchaintest"
