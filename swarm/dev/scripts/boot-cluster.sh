...
#
# The cluster contains a bootnode, a gkdchain node and multiple swarm nodes, with
...
# network namespaces togkdchainer
create_network() {
...
# start_bootnode starts a bootnode which is used to bootstrap the gkdchain and
# swarm nodes
start_bootnode() {
...
# start_gkdchain_node starts a gkdchain node with --datadir pointing at <base-dir>/gkdchain
# and a single, unlocked account with password "gkdchain"
start_gkdchain_node() {
  local dir="${base_dir}/gkdchain"
  mkdir -p "${dir}"

  local password="gkdchain"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating gkdchain account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gkdchain account address"
  fi
  ...
  start_node "gkdchain" "${GETH_IP}" "$(which gkdchain)" ${args[@]}
}
...
create_account() {
  ...
  gkdchain --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
