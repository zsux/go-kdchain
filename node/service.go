...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (kddb.Database, error) {
	if ctx.config.DataDir == "" {
		return kddb.NewMemDatabase()
	}
	db, err := kddb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
