...
import (
	...
	"github.com/kdchain/go-kdchain/internal/jsre"
	"github.com/kdchain/go-kdchain/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var kd = web3.kd; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = kd.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines kd.getBalance(kd.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	c.jsre.Run(`
		...
		console.log("coinbase: " + kd.coinbase);
		console.log("at block: " + kd.blockNumber + " (" + new Date(1000 * kd.getBlock(kd.blockNumber).timestamp) + ")");
