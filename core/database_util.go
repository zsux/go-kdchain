...
var (
	...
	configPrefix   = []byte("kdchain-config-") // config prefix for the db
...
func WriteCanonicalHash(db kddb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db kddb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db kddb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db kddb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db kddb.Putter, count uint64) error {
...
func WriteHeader(db kddb.Putter, header *types.Header) error {
...
func WriteBody(db kddb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db kddb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db kddb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db kddb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db kddb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db kddb.Putter, block *types.Block) error {
...
func WriteBloomBits(db kddb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db kddb.Database) kddb.Database {
	return kddb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db kddb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db kddb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db kddb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
