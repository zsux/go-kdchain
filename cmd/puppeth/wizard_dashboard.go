...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"kdstats", "explorer", "wallet", "faucet"} {
	...
	// If we have kdstats running, ask whether to make the secret public or not
	if w.conf.ethstats != "" {
		fmt.Println()
		fmt.Println("Include kdstats secret on dashboard (y/n)? (default = yes)")
