...
import (
	...
	"github.com/kdchain/go-kdchain/cmd/utils"
	"github.com/kdchain/go-kdchain/common"
	"github.com/kdchain/go-kdchain/console"
	"github.com/kdchain/go-kdchain/crypto"
	"github.com/kdchain/go-kdchain/kdclient"
	"github.com/kdchain/go-kdchain/internal/debug"
	...
	"github.com/kdchain/go-kdchain/swarm"
	bzzapi "github.com/kdchain/go-kdchain/swarm/api"
	swarmmetrics "github.com/kdchain/go-kdchain/swarm/metrics"
...
// This init function sets defaults so cmd/swarm can run alongside gkdchain.
func init() {
...
func init() {
	...
	app.Copyright = "Copyright 2013-2016 The go-kdchain Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.kdchain/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.kdchain/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.kdchain/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.kdchain/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//gkdchain only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gkdchain and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the kdchain node
	...
	//register BZZ as node.Service in the kdchain node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		var swapClient *kdclient.Client
		var err error
		if bzzconfig.SwapApi != "" {
			log.Info("connecting to SWAP API", "url", bzzconfig.SwapApi)
			swapClient, err = kdclient.Dial(bzzconfig.SwapApi)
			if err != nil {
	...
	//register within the kdchain node
