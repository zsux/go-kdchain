...
const (
	...
	ethVersion = 63 // equivalent kd version for the downloader
...
// NewProtocolManager returns a new kdchain sub protocol manager. The Ethereum sub protocol manages peers capable
// with the kdchain network.
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, protocolVersions []uint, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb kddb.Database, odr *LesOdr, txrelay *LesTxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case GetHeaderProofsMsg:
		...
		trieDb := trie.NewDatabase(kddb.NewTable(pm.chainDb, light.ChtTablePrefix))
		for _, req := range req.Reqs {
		...
		for _, req := range req.Reqs {
			if auxTrie == nil || req.Type != lastType || req.TrieIdx != lastIdx {
				...
				if root, prefix = pm.getHelperTrie(req.Type, req.TrieIdx); root != (common.Hash{}) {
					auxTrie, _ = trie.New(root, trie.NewDatabase(kddb.NewTable(pm.chainDb, prefix)))
				}
			}
