...
type wallet struct {
	...
	deriveChain    kdchain.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain kdchain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
