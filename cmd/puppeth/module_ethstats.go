...
// ethstatsDockerfile is the Dockerfile required to build an kdstats backend
// and associated monitoring site.
var ethstatsDockerfile = `
FROM puppeth/kdstats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// ethstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an kdstats monitoring site.
var ethstatsComposefile = `
version: '2'
services:
  kdstats:
    build: .
    image: {{.Network}}/kdstats{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new kdstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	// Build and deploy the kdstats service
	if nocache {
...
// ethstatsInfos is returned from an kdstats status check to allow reporting
...
// checkEthstats does a health-check against an kdstats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*ethstatsInfos, error) {
	// Inspect a possible kdstats container on the host
