...
// PeerInfo represents a short summary of the KDchain sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // KDchain protocol version negotiated
...
// Handshake executes the kd protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("kd/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the KDchain sub-protocol.
type peerSet struct {
