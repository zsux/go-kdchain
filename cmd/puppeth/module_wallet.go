...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gkdchain --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gkdchain --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --kdstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
